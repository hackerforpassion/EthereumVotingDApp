...
window.App = {
  ...
  start: function() { 
    ...
    VotingContract.deployed().then(function(instance){
	  ...
	  console.warn("no candidates! Adding default candidate list")
          ...
          numOfCandidates = 2 
    	    ...
    	    console.warn("Candidates count " + numOfCandidates)
	    ...
	    for (var i = 0; i < numOfCandidates; i++ ){
            ...
            instance.getCandidate(i).then(function(data){
              ...
              $("#candidate-box").append(`<div class="form-check"><input class="form-check-input" type="checkbox" value="" id=${data[0]}><label class="form-check-label" for=${data[0]}>${window.web3.toAscii(data[1])}</label></div>`)
    	       ...
    	       console.warn("Failed to add candidate")
	    })
          }
        }
...
window.addEventListener("load", function() {
  ...
  if (typeof web3 !== "undefined") {
    ...
    console.warn("No web3 detected. Falling back to http://localhost:8545. You should remove this fallback when you deploy live, as it's inherently insecure. Consider switching to Metamask for deployment. More info here: http://truffleframework.com/tutorials/truffle-and-metamask")
